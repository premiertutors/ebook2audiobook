...
class OmniVoiceUtilsTests(unittest.TestCase):
    def test_manifest_conversion_uses_normalized_text_and_language(self)->None:
    ...
    def test_configs_commands_and_step_calculation(self)->None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
    ...
    def test_checkpoint_discovery_and_packaging(self)->None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
