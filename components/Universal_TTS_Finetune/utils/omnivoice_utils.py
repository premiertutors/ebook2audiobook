...
def create_omnivoice_manifests(
    ...
    for split, metadata_path in split_sources.items():
        samples = (
            _read_ljspeech_metadata(metadata_path)
        ...
        if split == "train" and count == 0:
            raise ValueError("The prepared dataset contains no OmniVoice training samples.")
...
def build_omnivoice_configs(
    ...
    *,
    ...
    unused = {key: value for key, value in extra_overrides.items() if key not in allowed}
    train_config.update({key: value for key, value in extra_overrides.items() if key in allowed})
