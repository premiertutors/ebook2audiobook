...
class OmniVoiceEngine(TTSUtils, TTSRegistry, name='omnivoice'):
    def __init__(self, session:DictProxy)->None:
        try:
            ...
            engine_languages = default_engine_settings[TTS_ENGINES['OMNIVOICE']][
                'languages'
            ]
            if self.language not in engine_languages:
                raise ValueError(
                    f'Language {self.language} not supported by OmniVoice.'
                )
    ...
    def convert(self, sentence_file:str, sentence:str, **kwargs:Any)->tuple:
        try:
            import torch
